...
class ValueClauseTest extends AbstractTest
{
    ...
    {
        $clauses = array();
        $clause = new ValueClause(new StringMetadata('test_data1'));
        $clause->setTestedValue('value to use');
        $clauses['test_data1'] = $clause;

        $clause = new ValueClause(new FloatMetadata('test_data2'));
        $clause->setTestedValue(15.0);
        $clauses['test_data2'] = $clause;

        $clause = new ValueClause(new IntegerMetadata('test_data3'));
        $clause->setTestedValue(15);
        $clauses['test_data3'] = $clause;

        $clause = new ValueClause(new DateMetadata('test_data4'));
        $clause->setTestedValue('21-12-2017');
        $clauses['test_data4'] = $clause;

        $clause = new ValueClause(new URLMetadata('test_data5'));
        $clause->setTestedValue('http://www.test.com/123');
        $clauses['test_data5'] = $clause;
    ...
    public function testValueClauseValidationFailure()
    {
        ...
        $clause = new ValueClause($metadata);
        $clause->setTestedValue('value to use');
        ...
        $clause = new ValueClause(new DateMetadata('test_data4'));
        $clause->setTestedValue('07-2017');
        $clauses['data4'] = $clause;

        $clause = new ValueClause(new URLMetadata('test_data5'));
        $clause->setTestedValue('/www.test.com123');
        $clauses['data5'] = $clause;

        $clause = new ValueClause(new StringMetadata('test_data6'));
        $clause->setTestedValue(array('value to use'));
        $clauses['data6'] = $clause;

        $clause = new ValueClause(new NotIndexedMetadata('test_data7'));
        $clause->setTestedValue('value to use');
        $clauses['data7'] = $clause;
        ...
        foreach ($clauses as $testedData => $clause) {
            ...
            $this->assertNotEmpty($violations, 'ValueClause validation failed because it raises no error for: '.$testedData);
